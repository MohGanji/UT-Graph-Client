...
>>>>>>> a8eae5c3715b48e59888acd452235064e7619512
    ...
    let notifElement = newNotif.map(notif => {
      return <a href={`/notification/${notif.index}`}>{notif.message}</a>
    })
    if (this.state.notifications.length === 0)
      notifElement = (
        <p class="notification_box_empty"> اطلاعیه تازه ای ندارید! </p>
      )
    ...
    return (
      <div id="option_container" class="logged_in_option_container">
        <div
          ...
          id="invisible_box"
          class="notification_invisible"
        />
        <div
          onClick={this.showNotifications}
          class="logged_in_option_notification"
        >
          <div class={'notification_badge ' + hiddenClass}>
            {this.state.unReadNotifSize}
          </div>
          <div class="notification_icon">
            <i id="notification_icon" class="fa fa-bell" />
          </div>
          <div
            id="notifications_box"
            class="logged_in_option_notification_content"
          >
            {notifElement}
            <a href={`/notification`}>
